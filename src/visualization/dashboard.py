"""
Streamlit dashboard for the Neural Market Microstructure Predictor.
"""
...
from src.prediction.predictor import MarketPredictor
    ...
    if predict_button and selected_symbols and selected_models:
        with st.spinner("🔄 Loading models and generating predictions..."):
            try:
                ...
                # Load models
                if predictor.load_models(selected_models):
                    # Make predictions
                    predictions = predictor.predict_multiple_symbols(selected_symbols, selected_models)
...
def display_predictions(predictions, selected_models):
    ...
    summary_data = []
    for symbol, pred_data in predictions['symbols'].items():
        if 'ensemble' in pred_data:
            ensemble = pred_data['ensemble']
            summary_data.append({
                'Symbol': symbol,
                'Current Price': pred_data.get('current_price', 0),
                'Predicted Price': ensemble['predicted_price'],
                'Change': ensemble['price_change'],
                'Change %': ensemble['price_change_pct'],
        ...
        for i, row in df.iterrows():
            with cols[i]:
                ...
                <div class="metric-card">
                    <h3>{row['Symbol']} {direction_icon}</h3>
                    <p><strong>Current:</strong> ${row['Current Price']:.2f}</p>
                    <p><strong>Predicted:</strong> ${row['Predicted Price']:.2f}</p>
                    <p class="{direction_color}"><strong>Change:</strong> {row['Change %']:+.2f}%</p>
...
def create_model_comparison_chart(predictions, selected_models):
    """Create model comparison chart."""
    comparison_data = []
    
    for symbol, pred_data in predictions['symbols'].items():
        if 'models' in pred_data:
            for model_name, model_pred in pred_data['models'].items():
                comparison_data.append({
                    'Symbol': symbol,
                    'Model': model_name.upper(),
                    'Predicted Price': model_pred['predicted_price'],
                    'Price Change %': model_pred['price_change_pct']
                })
...
def create_price_visualization(df):
...
def show_sample_data():
    ...
    try:
        ...
        sample_data = collector.get_yahoo_data("AAPL", period="5d")
        
        if not sample_data.empty:
            # Show recent data
            st.dataframe(sample_data.tail(), use_container_width=True)
            ...
            fig.update_layout(
                title="AAPL - Recent Price Movement",
                xaxis_title="Date",
                yaxis_title="Price ($)",
                height=300
            )
