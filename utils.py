"""
Utility functions for the Neural Market Microstructure Predictor.
"""
...
def plot_predictions(actual: np.ndarray, predicted: np.ndarray, 
                    title: str = "Actual vs Predicted", figsize: tuple = (12, 6)):
    """
...
def plot_training_history(history: Dict, figsize: tuple = (15, 5)):
    """
    ...
    # MAPE
    if 'mape' in history:
        axes[2].plot(history['mape'], label='Training MAPE')
        if 'val_mape' in history:
            axes[2].plot(history['val_mape'], label='Validation MAPE')
