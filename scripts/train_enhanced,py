"""
Fixed data preprocessing module with correct technical analysis library usage.
"""
...
class DataPreprocessor:
    ...
    def add_technical_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        """Add technical indicators to the data with fixed library calls."""
        try:
            df = data.copy()
            ...
            # Volume indicators (Fixed - using correct functions)
            df['volume_sma'] = ta.trend.sma_indicator(df['volume'], window=20)  # Fixed: use trend.sma_indicator
            ...
            logger.info(f"Technical indicators added successfully. Total features: {len(df.columns)}")
            return df
    ...
    def create_sequences(self, data: pd.DataFrame, sequence_length: int = 60, 
        ...
        try:
            ...
            logger.info(f"Features used: {len(feature_columns)} - {feature_columns[:5]}..." if len(feature_columns) > 5 else f"Features: {feature_columns}")
            return X, y
    ...
    def preprocess_pipeline(self, data: pd.DataFrame, sequence_length: int = 60,
        ...
        try:
            ...
            if len(enhanced_data) < sequence_length + 50:  # Need minimum data
                logger.error(f"Insufficient data after preprocessing: {len(enhanced_data)} rows")
                return {}
            
            # Step 4: Create sequences
            X, y = self.create_sequences(enhanced_data, sequence_length, target_column)
